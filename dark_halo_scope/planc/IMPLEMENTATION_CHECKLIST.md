# Implementation Checklist - Tracking LLM Recommendations

**Created**: 2026-02-05  
**Last Updated**: 2026-02-05 (after LLM response)  
...
|----|------|--------|---------------|-------|
| P0.1 | Download DESI DR1 spectroscopic catalogs (single-fiber, pairwise) | ✅ VERIFIED | `data/external/desi_dr1/desi-sl-vac-v1.fits` | 2,176 entries, 1,603 high-quality. **Audited 2026-02-07** |
| P0.1b | Download DESI imaging candidates from lenscat | ✅ VERIFIED | `data/positives/desi_candidates.csv` | 5,104 entries (435 confident, 4,669 probable). **Audited 2026-02-07** |
| P0.2 | Crossmatch spectroscopic catalogs against imaging candidates | ✅ VERIFIED | `scripts/analyze_desi_sl_catalog.py` | **822 matches (16.1%)**, ~781 independent for validation. **Audited 2026-02-07** |
...
| P0.5 | Obtain/verify Tier-A anchor catalog completeness | ✅ VERIFIED | `data/positives/desi_candidates.csv` | **435 confident** from lenscat (grading="confident"). **Audited 2026-02-07** |
| P0.6 | Verify contaminant catalog categories are complete | ☐ NOT DONE | | Directory `data/contaminants/` does not exist. Use two-step: Tractor mining + image filtering |
...
| ID | Task | Status | File/Location | Notes |
|----|------|--------|---------------|-------|
| 1A.1 | Implement Pool N1: Deployment-representative sampling | ☐ PENDING | `emr/spark_negative_sampling.py` | TYPE in [SER,DEV,REX,EXP], z<20, stratified by nobs_z |
| 1A.2 | Implement Pool N2: Hard confuser sampling | ☐ PENDING | Same | Two-step: Tractor mining + image filtering |
| 1A.3 | Define N1:N2 ratio in training | 📋 DECISION MADE | | **85:15** within negative class |
| 1A.4 | Implement 100:1 negative:positive ratio per (nobs_z, type) bin | ☐ PENDING | | ~85 from N1, ~15 from N2 per stratum |
| 1A.5 | Define control sample handling | 📋 DECISION MADE | | **Deprecate as primary negative**, keep as diagnostic only |
| 1A.6 | Implement galaxy de-duplication across sweep files | ☐ PENDING | | Prevent same galaxy appearing multiple times |
| 1A.7 | Implement exclusion of known/candidate lenses from negatives | ☐ PENDING | | **5" + 2×θE_max (~11")** from lens center |

### 1B. Spatial Splits
| ID | Task | Status | File/Location | Notes |
|----|------|--------|---------------|-------|
| 1B.1 | Integrate HEALPix splitting (replace hash-based) | ☐ PENDING | `emr/spark_phase3_pipeline.py` lines 899-910 | Use `phase8_healpix_splits.py` code |
| 1B.2 | Choose nside | 📋 DECISION MADE | | **nside=128** for safer "independent regions" claims |
| 1B.3 | Stratify HEALPix cells by observing conditions | 📋 DECISION MADE | | **Don't hard-stratify**, just verify and report balance |
| 1B.4 | Implement 70/15/15 train/val/test allocation | ☐ PENDING | | Deterministic hash on healpix_index |
| 1B.5 | Verify spatial disjointness (no healpix cell in multiple splits) | ☐ PENDING | | Apply same function to positives and negatives |

### 1C. Schema Implementation
| ID | Task | Status | File/Location | Notes |
|----|------|--------|---------------|-------|
| 1C.1 | Level 1: Galaxy Catalog Manifest - core columns | ☐ PENDING | | galaxy_id, brickname, ra, dec, type |
| 1C.2 | Level 1: Stratification columns | ☐ PENDING | | nobs_z, nobs_z_bin, type_bin |
| 1C.3 | Level 1: Photometry columns | ☐ PENDING | | flux_g/r/z/w1, mag_g/r/z, colors |
| 1C.4 | Level 1: Observing condition columns | ☐ PENDING | | psfsize_g/r/z, psfdepth_g/r/z, galdepth_g/r/z, ebv |
| 1C.5 | Level 1: Quality flag columns | ☐ PENDING | | maskbits, fitbits, mw_transmission |
| 1C.6 | Level 1: Spatial/split columns | ☐ PENDING | | healpix_64, healpix_128, region_split |
| 1C.7 | Level 1: Provenance columns | ☐ PENDING | | sweep_file, row_index, pipeline_version, git_commit |
