...
def azimuthal_shuffle(
    ...
    # Bin edges - add small epsilon to ensure corner pixels are included
    # This fixes the bug where pixels at exactly r==r_max were missed
    r_max = float(r.max()) + 1e-6
    bin_edges = np.linspace(0.0, r_max, n_bins + 1, dtype=np.float32)
    ...
    for i in range(n_bins):
        if i == n_bins - 1:
            # Last bin: include the upper boundary to ensure all pixels covered
...
def apply_core_dropout(
    ...
    IMPORTANT: Fill statistics are computed from the OUTER ANNULUS only
    (r > OUTER_RADIUS_PIX), not from ~core_mask. This prevents arc signal
    from contaminating the fill values.
    ...
    # Use outer annulus (r > OUTER_RADIUS_PIX) for fill statistics
    # This avoids including arc region which is typically at r ~ theta_E
    outer_annulus_mask = create_radial_mask(h, w, OUTER_RADIUS_PIX, inside=False)
    
    result = img.copy()
    
    for i in range(c):
        if fill_mode == "outer_median":
            fill_value = np.median(img[i][outer_annulus_mask])
            result[i][core_mask] = fill_value
        elif fill_mode == "zero":
            result[i][core_mask] = 0.0
        elif fill_mode == "noise":
            outer_std = np.std(img[i][outer_annulus_mask])
            outer_mean = np.mean(img[i][outer_annulus_mask])
