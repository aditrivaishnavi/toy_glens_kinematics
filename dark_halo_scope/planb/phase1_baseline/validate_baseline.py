...
def compute_core_lr_auc(
    ...
    IMPORTANT: Uses out-of-fold cross-validation to avoid train-on-eval bias.
    ...
    # Use out-of-fold cross-validation to compute AUC
    # This prevents train-on-eval bias which inflates the AUC
    clf = LogisticRegression(max_iter=1000, solver="liblinear", random_state=42)
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    
    probs_oof = np.zeros(len(y), dtype=np.float32)
    for train_idx, test_idx in skf.split(X, y):
        clf.fit(X[train_idx], y[train_idx])
        probs_oof[test_idx] = clf.predict_proba(X[test_idx])[:, 1]
    
    return roc_auc_score(y, probs_oof)
