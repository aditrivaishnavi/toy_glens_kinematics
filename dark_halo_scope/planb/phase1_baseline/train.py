...
def evaluate_gates(
    ...
    # Core LR AUC - use cross-validation to avoid train-on-eval bias
    # This ensures the AUC measures generalizable separability, not overfit
    ...
    probs_oof = np.zeros(len(y_core), dtype=np.float32)
    for train_idx, test_idx in skf.split(X_core, y_core):
        clf.fit(X_core[train_idx], y_core[train_idx])
        probs_oof[test_idx] = clf.predict_proba(X_core[test_idx])[:, 1]
    
    results["core_lr_auc"] = roc_auc_score(y_core, probs_oof)
...
def train(
    ...
    logger.info(f"Save every epoch: {save_every_epoch}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        # Save periodic checkpoint (every 5 epochs by default, or every epoch if flag set)
        should_save = save_every_epoch or (epoch % 5 == 0)
        if should_save:
            torch.save({
                "epoch": epoch,
                "model_state_dict": model.state_dict(),
    ...
    # Load best model
    checkpoint = torch.load(f"{output_dir}/best_model.pt", weights_only=False)
...
def main():
    ...
    parser.add_argument("--save-every-epoch", action="store_true", 
                        help="Save checkpoint every epoch (default: every 10)")
