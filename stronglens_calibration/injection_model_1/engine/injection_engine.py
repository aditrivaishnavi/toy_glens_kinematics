...
def _sersic_shape(
    ...
    # b_n approximation keeps Re as half-light radius
    b_n = 2.0 * n - 1.0 / 3.0
...
def inject_sis_shear(
    ...
    host_nmgy_hwc: (H,W,3) torch float32 in nanomaggies.
    ...
    Returns injected and injection-only tensors in CHW.
    """
...
def sample_source_params(
    ...
    re_arcsec_range: Tuple[float, float] = (0.05, 0.25),
    n_range: Tuple[float, float] = (0.7, 2.5),
...
def estimate_sigma_pix_from_psfdepth(
    ...
    psfdepth is inverse variance of PSF-flux (nmgy^-2).
    """
