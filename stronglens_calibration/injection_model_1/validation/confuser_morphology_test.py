...
def load_model(checkpoint_path: str, device: torch.device):
    """Load model from checkpoint using same pattern as other scripts."""
    try:
...
# ---------------------------------------------------------------------------
@torch.no_grad()
def score_cutout(model: nn.Module, cutout_path: str, device: torch.device) -> Optional[float]:
...
def score_batch(
    ...
    for i, path in enumerate(cutout_paths):
        p = score_cutout(model, path, device)
        if p is not None:
            scores.append(p)
        else:
...
def main():
    ...
    if has_confuser:
        for cat in CONFUSER_CATEGORIES:
            ...
            scores, n_err = score_batch(model, paths, device)
    ...
    baseline_scores, n_err = score_batch(model, baseline_paths, device)
