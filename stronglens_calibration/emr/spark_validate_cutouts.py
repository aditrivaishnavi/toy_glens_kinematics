...
def validate_cutout_quality(cutout: np.ndarray) -> Dict:
    ...
    return {
        ...
        "core_nan_ok": core_nan_ok,
        # Band-wise NaN (LLM recommendation)
        "nan_frac_g": band_nan_fracs[0] if len(band_nan_fracs) > 0 else None,
        "nan_frac_r": band_nan_fracs[1] if len(band_nan_fracs) > 1 else None,
        "nan_frac_z": band_nan_fracs[2] if len(band_nan_fracs) > 2 else None,
        "central_nan_g": central_band_nan[0] if len(central_band_nan) > 0 else None,
        "central_nan_r": central_band_nan[1] if len(central_band_nan) > 1 else None,
        "central_nan_z": central_band_nan[2] if len(central_band_nan) > 2 else None,
        # Band presence
...
def extract_shortcut_features(cutout: np.ndarray) -> Dict:
    ...
    if arc_brightness is not None and len(valid_arc) > 10:
        # Simple approach: std of arc pixels normalized by median
        azimuthal_asymmetry = float(np.std(valid_arc) / (arc_brightness + 1e-10))
...
def main():
    ...
    try:
        ...
        # Process in parallel
        pos_rdd = sc.parallelize([(b, k, "positive") for b, k in pos_files], numSlices=100)
        neg_rdd = sc.parallelize([(b, k, "negative") for b, k in neg_files], numSlices=200)
        ...
        shortcut_features = [
            ...
            # Artifacts
            "saturation_frac", "edge_artifact_score",
            # Noise
            "mad_r",
        ]
        ...
        for feature in shortcut_features:
            ...
            auc_result = compute_auc(pos_vals, neg_vals, n_bootstrap=200)
            auc = auc_result["auc"]
            ...
            shortcut_report["features"][feature] = {
                ...
                "auc_lo": auc_lo,
                "auc_hi": auc_hi,
                "is_shortcut": is_red,  # Backward compat
                "is_red": is_red,
                "is_yellow": is_yellow,
            ...
            logger.info(f"  {feature}: AUC={auc:.3f} [{auc_lo:.3f}, {auc_hi:.3f}] {flag}")
        ...
        # Count yellow and red flags
        n_red = sum(1 for f in shortcut_report["features"].values() if f.get("is_red", False))
        n_yellow = sum(1 for f in shortcut_report["features"].values() if f.get("is_yellow", False))
        
        shortcut_report["gates"] = {
            "no_core_shortcuts": not core_shortcuts,
            "annulus_separates": annulus_auc > 0.55,  # Should be separable
            "no_trivial_shortcuts": n_red == 0,  # No features with AUC_lo > 0.70
            "few_yellow_flags": n_yellow <= 3,  # At most 3 yellow flags
        }
