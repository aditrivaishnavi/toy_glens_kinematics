...
All files are packaged in `prompt_12_paper_review_package.zip` (813K). Contents:

- **Paper draft** (LaTeX + compiled PDF): `paper/mnras_merged_draft_v1.tex`, `paper/mnras_merged_draft_v1.pdf`
- **D05 verified results** (JSON/CSV): all 12 result files in `results/D05_20260214_full_reeval/` (6 bright-arc JSONs, 2 grid CSVs, 2 grid meta JSONs, linear probe JSON, Tier-A scoring JSON)
- **Core code** for methodology verification: `dhs/injection_engine.py` (Poisson implementation lines 549-567), `scripts/selection_function_grid.py`, `scripts/feature_space_analysis.py`, `sim_to_real_validations/bright_arc_injection_test.py`
- **Injection priors**: `configs/injection_priors.yaml`
