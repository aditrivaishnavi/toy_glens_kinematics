...
\begin{enumerate}
\item A quantitative measurement of the injection realism gap for a CNN lens finder on ground-based survey cutouts: even for brightness-matched injections at lensed magnitudes $19$--$22$, detection rates reach only $29$--$39$ per cent (Section~\ref{sec:brightarc}; $n = 200$ per magnitude bin, $\beta_{\rm frac} \in [0.10, 0.40]$) versus $89.3$ per cent Tier-A recall ($5.18$ per cent when averaged over the full parameter space).
...
Fr\'{e}chet distance (features\_1, 32-d) & 1.45 \\
Fr\'{e}chet distance (features\_2, 48-d) & 10.9 \\
Fr\'{e}chet distance (features\_3, 160-d) & 47.2 (directional only) \\
...
Table~\ref{tab:brightarc} presents the full detection-rate matrix.  The baseline detection rate peaks at mag~$20$--$21$ ($38.5$~per cent), declining at the brightest magnitudes ($29.0$~per cent at mag~$18$--$19$).  This decline is explained by the clip$\,=\,$10 preprocessing: arc pixels brighter than 10 normalised units are truncated to the clip ceiling, collapsing their curved morphology into a flat plateau indistinguishable from saturated artefacts.  The clip$\,=\,$20 results confirm this mechanism, substantially increasing bright-arc detection (mag~$18$--$19$: $29.0 \to 45.0$~per cent).

Contrary to the first-principles prediction of universal degradation, Poisson noise has a \emph{regime-dependent} effect.  At intermediate magnitudes ($20$--$23$), Poisson noise \emph{increases} detection: $+3.0$~pp at mag~$20$--$21$ ($38.5 \to 41.5$~per cent), $+10.5$~pp at mag~$21$--$22$ ($33.0 \to 43.5$~per cent), and $+9.0$~pp at mag~$22$--$23$ ($23.5 \to 32.5$~per cent).  At the brightest and faintest magnitudes, Poisson has no significant effect (mag~$18$--$19$: both $29.0$~per cent; mag~$25$--$26$: both $0.5$~per cent).  A paired per-injection analysis confirms that the increase at mag~$21$--$22$ is a systematic score uplift (mean $\Delta p = +0.089$, with 27 injections crossing the $p = 0.3$ threshold upward and only 6 downward), not merely threshold scatter.  The median score nearly doubles at this bin (from $0.085$ to $0.161$), indicating that Poisson noise adds physically correct pixel-level texture to the smooth S\'{e}rsic arc, making it less distinguishable from real sky signal.
...
\item[(iii)] A linear probe in the CNN's penultimate feature space separates real lenses from brightness-matched injections with AUC $= 0.997 \pm 0.003$, establishing that the CNN has learned to distinguish them based on properties beyond brightness alone.  The Fr\'{e}chet distance grows $330\times$ from the earliest to mid-level network layers (0.14 to 47.2), demonstrating that the morphological barrier is encoded in learned mid-level features (texture, shape), not merely low-level pixel statistics.

\item[(iv)] Adding arc-level Poisson noise to injections has a \emph{regime-dependent} effect on detection: it reduces marginal grid completeness (from $5.18$ to $3.79$~per cent, $z = 15.7$, $p < 10^{-50}$) but increases detection of moderately bright arcs (mag~$21$--$22$: $+10.5$~pp) and compact arcs ($\theta_{\rm E} \leq 1.25$: up to $+0.81$~pp).  The crossover between these regimes occurs near $\theta_{\rm E} \approx 1.3$.  A paired per-injection analysis confirms that the increase at mag~$21$--$22$ is a systematic score uplift (mean $\Delta p = +0.089$), not threshold scatter.  We interpret this as a dual mechanism: Poisson noise adds realistic texture (aiding detection when arcs are marginally detectable) but degrades spatial coherence (harming detection when arcs are geometrically prominent).  A gain sweep control at $10^{12}\;\mathrm{e^{-}\;nmgy^{-1}}$ recovers the no-noise baseline, confirming the result is physical.

\item[(v)] Under our simplified PSF model, the injection realism gap is consistent with a \emph{morphological barrier}: parametric S\'{e}rsic profiles lack the spatially coherent substructure of real lensed galaxies.  Shot noise plays a secondary, regime-dependent role but is insufficient to close the gap.  The injection completeness map $C(\theta_{\rm E}, \mathrm{PSF}, \mathrm{depth})$ should be interpreted as completeness for the specific parametric injection family, not as an unbiased estimate of the true survey selection function.  We propose the linear probe AUC as a quantitative realism gate for the community to evaluate and compare injection pipelines.
\end{enumerate}
