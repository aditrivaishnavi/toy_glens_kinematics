...
\caption{Independent galleries of real strong lenses and parametric injections, drawn from separate datasets with no pairing or brightness matching.  \textbf{(a)}~Six randomly sampled Tier-A validation lenses, sorted by $r$-band magnitude.  \textbf{(b)}~Six randomly sampled S\'{e}rsic injections from the D06 grid (no Poisson) at $\theta_{\rm E} = 0.75$, $1.50$, $2.50\;\mathrm{arcsec}$ (two per regime).  Each thumbnail shows the CNN detection probability~$p$ and $r$-band magnitude.  Real lenses span a range of scores, while injections are almost uniformly rejected ($p \approx 0$), consistent with the $5.18$~per~cent grid completeness (Table~\ref{tab:thresholds}).  Cutouts are $101 \times 101$~pixels in $grz$.}
