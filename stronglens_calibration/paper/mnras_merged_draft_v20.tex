...
\begin{table}
\centering
\caption{The 12 Tier-A lenses missed by the CNN at the $p > 0.3$ detection threshold.  Sorted by ascending CNN score.  $z_{\rm lens}$ from the DESI Strong Lensing catalogue cross-matched by position ($< 12\;\mathrm{arcmin}$ from the brick centre; the large matching radius reflects the offset between the Legacy Survey brick centre and the lens position within the $\sim\!0.25\;\mathrm{deg}$ brick, not a positional uncertainty).  All missed lenses have deflector $r$-band magnitudes $\leq 20$.}
\label{tab:missed}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llccl}
