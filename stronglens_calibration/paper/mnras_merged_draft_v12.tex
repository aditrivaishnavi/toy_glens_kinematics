...
Standard injection-recovery using parametric S\'{e}rsic source profiles lensed by a singular isothermal ellipsoid yields a marginal completeness of only $5.18$ per cent ($5697/110\,000$) over the full parameter space, which is dominated by faint ($m_{\rm lensed} > 22$) configurations.  Even when restricted to bright, favourably lensed injections at source magnitudes $m_{\rm source} = 18\text{--}22$ (corresponding to lensed arc magnitudes $\sim\!1$--$2.5$~mag brighter after gravitational magnification), detection rates reach only $29$--$39$ per cent (Section~\ref{sec:brightarc}; $n = 200$ per magnitude bin) --- a factor of $2$--$3$ below the Tier-A recall.  A linear probe (logistic regression) trained on the CNN's penultimate 1280-dimensional features separates real lenses from brightness-matched injections with AUC $= 0.997 \pm 0.003$ (five-fold cross-validation), indicating that the deficit reflects a strong feature-space separation that persists beyond brightness differences, consistent with a morphological mismatch between parametric and real arc morphology (though host-galaxy population differences may also contribute; see Section~5.4).
...
\caption{Visual comparison of real Tier-A lenses (odd columns) and brightness-matched parametric injections (even columns) at three Einstein radii: $\theta_{\rm E} = 0.75$ (compact), $1.50$ (medium), and $2.50\;\mathrm{arcsec}$ (extended).  Each pair is matched to within $< 0.1$~mag in total $r$-band flux.  Despite appearing visually similar --- especially in the compact regime --- the CNN scores the real lenses at $p > 0.99$ while scoring the brightness-matched injections at $p < 0.01$ in most cases.  This illustrates the morphological barrier: the differences between parametric S\'{e}rsic injections and real lensed galaxies are subtle to the eye but readily detected by the CNN's learned mid-level features (Section~4.3).  Cutouts are $101 \times 101$ pixels ($26.5 \times 26.5\;\mathrm{arcsec}$) in $grz$ composite, from the D06 grid (no-Poisson baseline).}
...
Contrary to the photoelectron-budget prediction of universal degradation, Poisson noise has a \emph{regime-dependent} effect.  At intermediate magnitudes ($20$--$23$), Poisson noise \emph{increases} detection: $+3.0$~pp at mag~$20$--$21$ ($38.5 \to 41.5$~per cent), $+10.5$~pp at mag~$21$--$22$ ($33.0 \to 43.5$~per cent), and $+9.0$~pp at mag~$22$--$23$ ($23.5 \to 32.5$~per cent).  At the brightest and faintest magnitudes, Poisson has no significant effect (mag~$18$--$19$: both $29.0$~per cent; mag~$25$--$26$: both $0.5$~per cent).  A paired per-injection analysis confirms that the increase at mag~$21$--$22$ is a systematic score uplift (mean $\Delta p = +0.089$, with 27 injections crossing the $p = 0.3$ threshold upward and only 6 downward), not merely threshold scatter.  The median score nearly doubles at this bin (from $0.085$ to $0.161$), consistent with Poisson noise adding realistic pixel-level texture to the smooth S\'{e}rsic arc, making it less distinguishable from real sky signal.

Table~\ref{tab:paired_delta} reports the full paired per-injection analysis for all eight magnitude bins, including sign-test $p$-values.  The Poisson increase is statistically significant at mag~$21$--$22$ ($p = 3.2 \times 10^{-4}$, binomial sign test on 27 vs 6 threshold crossings) and mag~$22$--$23$ ($p = 2.8 \times 10^{-4}$, 21 vs 3 crossings).  Both $p$-values survive Bonferroni correction for eight bins (threshold $0.05/8 = 0.00625$).  The effect at mag~$20$--$21$ ($+3.0$~pp, $p = 0.45$) is not individually significant at the 200-injection sample size; the aggregate trend across three contiguous bins supports the interpretation as a real effect.
